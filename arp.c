...
struct arp_hdr
{
  uint16_t hrd; // hw address type
  uint16_t pro; // protocol address type
  uint8_t hln;  // hw address length
...
// entity of arp table
// thread unsafe, must use with mutex
...
struct arp_ether_ip
{
  struct arp_hdr hdr;
  /* variable range */
  uint8_t sha[ETHER_ADDR_LEN]; // sender hw address
  uint8_t spa[IP_ADDR_LEN];    // sender protocol address
  uint8_t tha[ETHER_ADDR_LEN]; // target hw address
...
arp_cache_alloc(void)
{
  ...
  for (entry = caches; entry < tailof(caches); entry++)
  {
    ...
    // search oldest entry in case can't find empty entry
