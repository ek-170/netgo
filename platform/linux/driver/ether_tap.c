...
struct ether_tap
{
  char name[IFNAMSIZ]; // name of TAP device
  int fd;
...
{
  /*
    Opening a socket even though you are not communicating with it,
    since ioctl's SIOCGIFHWADDR request is only valid for descriptors opened as sockets
  */
  int soc;
  // https://man7.org/linux/man-pages/man7/netdevice.7.html
  struct ifreq ifr = {}; // use for ioctl
  ...
  // SIOCGIFHWADDR: request code of get MAC address
...
ether_tap_open(struct net_device *dev)
{
  ...
  // if hw address is not set implicitly
  if (memcmp(dev->addr, ETHER_ADDR_ANY, ETHER_ADDR_LEN) == 0)
  {
    // get and use hw address which is seen from os
...
// this is called when isr signal recieved
// irq is irq number
// id is net_device(set when called intr.c: intr_thread())
static int
...
// generate net_device of ethernet, setup common helper parameters and driver ops
struct net_device *
ether_tap_init(const char *name, const char *addr)
{
  ...
  if (addr)
  {
    ...
    {
      errorf("invalid address,m addr=%s", addr);
      return NULL;
    }
  }
  ...
  tap->fd = -1; // invalid initial value
  tap->irq = ETHER_TAP_IRQ;
  dev->priv = tap; // set ether_tap struct instance to device's private area
