...
// udp recieved
// data is UDP header and data(without ip payload)
// len is UDP header and data total length(without ip payload)
static void
...
// this is simulation of socket open
int udp_open(void)
{
  struct udp_pcb *pcb;
  int id;
...
// this is simulation of systemcall bind
int udp_bind(int id, struct ip_endpoint *local)
{
  ...
  mutex_lock(&mutex);
  // Associate UDP socket with addresses and port numbers
  pcb = udp_pcb_get(id);
  if (!pcb)
  {
    errorf("pcb not found, id=%d", id);
...
// polling udp entry queue and if data come, copy to *buf
ssize_t
udp_recvfrom(int id, uint8_t *buf, size_t size, struct ip_endpoint *foreign)
{
  ...
  while (1)
  {
    ...
    // Wait to be woken up by sched_wakeup() or sched_interruppt()
    err = sched_sleep(&pcb->ctx, &mutex, NULL);
    if (err)
    {
      // this err is by sched_interrup()
      debugf("interrupted");
      mutex_unlock(&mutex);
      errno = EINTR;
      return -1;
